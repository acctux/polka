...
local function load_locale_file()
	if user_opts.language == "default" or locale_file_loaded then
		return
	end
...
local function set_osc_styles()
	...
	osc_styles = {
		...
		thumbnail = "{\\bord" .. user_opts.thumbnail_box_outline_size .. "\\1c&H" .. osc_color_convert(
			user_opts.thumbnail_box_color
		...
		control_mini = "{\\1c&H"
			...
			.. icons.iconfont
			.. "}",
		element_down = "{\\1c&H" .. osc_color_convert(user_opts.held_element_color) .. "&" .. string.format(
			"\\fscx%s\\fscy%s",
		...
		element_hover = "{"
			...
			.. (hover_effects.size and string.format(
				"\\fscx%s\\fscy%s",
				user_opts.button_hover_size,
				user_opts.button_hover_size
			) or "")
			.. "}",
...
local function get_virt_scale_factor()
	...
	return osc_param.playresx / state.osd_dimensions.w, osc_param.playresy / state.osd_dimensions.h
end
...
local function get_time_codes_width()
	...
	local rt_sec = state.tc_left_rem and mp.get_property_number("playtime-remaining", 0)
		or mp.get_property_number("playback-time", 0)
...
local function prepare_elements()
	...
	for _, element in pairs(elements) do
		...
		if (element.name == "title" or element.name == "chapter_title") and type(element.content) == "function" then
			...
			if width > 0 then
				hitbox_w = math.min(width, hitbox_w)
			end
		end
...
local function draw_gap_segments(elem_ass, element, gap_half, x_max, slider_lo, elem_geo, radius)
	...
	if x_max > seg_start then
		elem_ass:round_rect_cw(
			...
			elem_geo.h - slider_lo.gap,
			(seg_start == 0) and radius or 0,
			(x_max == elem_geo.w) and radius or 0
		)
	end
end
...
local function draw_seekbar_ranges(element, elem_ass, xp, rh, override_alpha, inverse)
	...
	for _, range in pairs(seekRanges) do
		...
		for _, cut in ipairs(cuts) do
			...
			if gap_r > pstart and gap_l < pend then
				...
				if handle and (sl < xp + rh and sr > xp - rh) then
					...
					if xp + rh < sr then
						elem_ass:round_rect_cw(xp + rh, slider_lo.gap, sr, elem_geo.h - slider_lo.gap, 0, 0)
					end
				elseif sr > sl then
					elem_ass:round_rect_cw(
						sl,
		...
		if handle and (sl < xp + rh and sr > xp - rh) then
			if sl < xp - rh then
				elem_ass:round_rect_cw(sl, slider_lo.gap, xp - rh, elem_geo.h - slider_lo.gap, rl, 0)
			end
			if xp + rh < sr then
				elem_ass:round_rect_cw(xp + rh, slider_lo.gap, sr, elem_geo.h - slider_lo.gap, 0, r_right)
			end
		else
			if sr > sl then
				elem_ass:round_rect_cw(sl, slider_lo.gap, sr, elem_geo.h - slider_lo.gap, rl, r_right)
			end
		end
	end
end
...
local function draw_ab_loop_range(element, elem_ass)
	...
	if not state.duration or not ab_a or ab_a < 0 then
		return
	end
	...
	local bx = (ab_b and ab_b > ab_a and ab_b <= state.duration)
			and get_slider_ele_pos_for(element, ab_b / state.duration * 100)
		or elem_geo.w
	if ax >= bx then
		return
	end
...
local function draw_seekbar_nibbles(element, elem_ass)
	...
	for n, marker in ipairs(markers) do
		if
			n > 1
			and (n - 1) ~= current_chapter
			and marker >= element.slider.min.value
...
local function render_elements(master_ass, osc_vis, wc_vis)
	local function render_element(n)
		...
		then
			...
			if
				(hb and mouse_hit_coords(hb.x1, hb.y1, hb.x2, hb.y2) or mouse_hit(element))
				and is_clickable
				and element.enabled
			then
				local hx1, hy1, hx2, hy2
				if hb then
					hx1, hy1, hx2, hy2 = hb.x1, hb.y1, hb.x2, hb.y2
				else
					hx1, hy1, hx2, hy2 = get_element_hitbox(element)
				end
				...
				elem_ass:pos(0, 0)
				elem_ass:an(7)
				local hover_base_alpha = state.mouse_down_counter > 0
						and (255 - math.floor(math.max(0, math.min(100, user_opts.button_held_box_alpha)) * 2.55))
					or (element.hover_alpha or 0xE6)
				ass_append_alpha(
					elem_ass,
				...
				if element.slider and element.slider.tooltipF ~= nil and element.enabled then
					...
					if mouse_hit(element) or force_seek_tooltip then
						...
						if an == 2 then
							...
							local ref_el = (element.name == "volumebar" and seekbar and seekbar.hitbox) and seekbar
								or element
						...
						if element.thumbnailable and not thumbfast.disabled and osd_w then
							...
							local thumb_radius = user_opts.thumbnail_box_radius > 0 and user_opts.thumbnail_box_radius
								or 0

							local hover_sec = 0
							if state.duration then
								hover_sec = state.duration * sliderpos / 100
							end

							local margin_x = 18 / r_w
							local thumb_x = math.min(
								osd_w - thumbfast.width - margin_x,
								math.max(margin_x, tx / r_w - thumbfast.width / 2)
							)
		...
		elseif element.type == "button" then
			...
			if
				element.tooltipF ~= nil
				and element.enabled
				and not seeking_with_force_tooltip
				and user_opts.tooltip_hints
			then
				local hb = element.hover_box
				if hb and mouse_hit_coords(hb.x1, hb.y1, hb.x2, hb.y2) or mouse_hit(element) then
					...
					local seekbar_ref = (state.seekbar_element and state.seekbar_element.hitbox)
							and state.seekbar_element
						or element
...
local function window_controls()
	...
	local lo
	local ontop_active = user_opts.ontop_button
		and window_controls_enabled()
		and user_opts.ontop_in_topbar
		and state.ontop
	...
	if user_opts.window_controls then
		local size_hover = hover_effects.size
				and string.format("\\fscx%s\\fscy%s", user_opts.button_hover_size, user_opts.button_hover_size)
			or ""
	...
	if user_opts.show_window_title or user_opts.window_controls or ontop_active then
		...
		if ontop_active then
			add_area("window-controls-ontop", 0, 0, 50, wc_geo.h)
		end
...
layouts["default"] = function()
	...
	local title_offset = (no_chapter or not chapter_index or user_opts.chapter_above_title) and user_opts.title_offset
		or user_opts.title_with_chapter_offset
	...
	if title_and_chapter_h_with_offset == 0 then
		...
		local ch_skip = user_opts.chapter_skip_buttons
			and type(state.chapter_list) == "table"
			and next(state.chapter_list)
		local outer = (ch_skip and 0 or 100) + (user_opts.jump_buttons and 0 or 100)
		local timecodes_above = osc_param.playresx
			< (
		...
		title_and_chapter_h_with_offset = timecodes_above
				and math.max(
					user_opts.osc_height * 0.2,
					user_opts.time_codes_offset + user_opts.title_offset + user_opts.time_font_size
				)
	...
	local ontop_button = user_opts.ontop_button
		and not (window_controls_enabled() and user_opts.ontop_in_topbar and state.ontop)
	local playlist_button = user_opts.playlist_button
		and (not user_opts.hide_empty_playlist_button or state.playlist_count > 1)
	...
	if user_opts.chapter_above_title then
		...
		title_y = (no_chapter or not chapter_index) and (user_opts.osc_height + title_offset)
			or (chapter_title_y + chapter_h + user_opts.title_with_chapter_offset)
	end

	-- osc title
	local title_w = (no_chapter or not chapter_index or user_opts.chapter_above_title)
			and (osc_geo.w - 60 - time_codes_width)
		or (osc_geo.w - 50)
	...
	if user_opts.show_chapter_title then
		...
		lo.alpha[3] = 0
		lo.style =
			string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.chapter_title, 0, 0, geo.x + geo.w, geo.y + geo.h)
	...
	if audio_track and user_opts.volume_control then
		...
		lo.slider.handle_color = user_opts.volumebar_match_seek_color and user_opts.seekbarfg_color
			or user_opts.side_buttons_color
		lo.slider.gap = 3
		...
		elements["vol_ctrl"].hover_box = vol_vis
				and { x1 = vc_left, y1 = osc_mid - 12, x2 = vc_left + 87, y2 = osc_mid + 12 }
			or nil
	end
	...
	if user_opts.track_nextprev_buttons then
		elements["playlist_prev"].visible = (
			state.playlist_count > 1 or contains(user_opts.buttons_always_active, "playlist_prev")
		) and (osc_param.playresx >= 500 - outeroffset)
	...
	if user_opts.track_nextprev_buttons then
		elements["playlist_next"].visible = (
			state.playlist_count > 1 or contains(user_opts.buttons_always_active, "playlist_next")
		) and (osc_param.playresx >= 500 - outeroffset)
...
layouts["seekbar"] = function()
	...
	lo.geometry = {
		x = osc_geo.w - 25,
		y = refY - first_row_y - second_row_y,
		an = 3,
		w = time_codes_width,
		h = user_opts.time_font_size,
	}
...
layouts["modern-image"] = function()
	...
	-- left side
	if playlist_button then
		left_side_button("playlist", 25, 250, nil, osc_styles.control_3)
	end
	...
	if fullscreen_button then
		right_side_button("fullscreen", 350)
	end
	if info_button then
		right_side_button("info", 400)
	end
	if ontop_button then
		right_side_button("ontop", 450)
	end
	if user_opts.download_button then
		right_side_button("download", 500, state.is_url)
	end
end
...
local function osc_init()
	...
	local function truncate_title(title, max_w, style)
		if not max_w or max_w <= 0 or estimate_text_width(title, style) <= max_w then
			return title
		end
	...
	ne.content = function()
		...
		local chapter_title = mp.command_native({
			"escape-ass",
	...
	ne.content = function()
		return state.eof_reached and icons.replay
			or (state.pause and not state.playing_and_seeking and icons.play)
			or icons.pause
	end
	...
	ne.tooltipF = function()
		return have_pl and locale.playlist .. " [" .. pl_pos .. "/" .. pl_count .. "]"
			or locale.playlist .. " / " .. locale.menu
	end
	...
	local function track_tooltip(label, track_type, id_prop, count)
		local prop = mp.get_property("current-tracks/" .. track_type .. "/title")
			or mp.get_property("current-tracks/" .. track_type .. "/lang")
			or locale.unknown
	...
	ne.tooltipF = function()
		return track_tooltip(locale.audio, "audio", "aid", state.audio_track_count)
	end
	...
	ne.tooltipF = function()
		return track_tooltip(locale.subtitle, "sub", "sid", state.sub_track_count)
	end
	...
	ne.content = function()
		...
		return state.mute and icons.volume_mute
			or (volume >= 75 and icons.volume_high)
			or (volume >= 25 and icons.volume_low)
			or icons.volume_quiet
	end
	ne.tooltipF = function()
		local volume = state.volume
	...
	local function zoom_step(delta)
		...
		mp.commandv(
			...
			"set",
			"video-zoom",
			math.max(user_opts.zoom_out_min, math.min(user_opts.zoom_in_max, z + delta))
		)
	end
	...
	ne.tooltipF = function()
		return state.fullscreen and locale.fullscreen_exit or locale.fullscreen
	end
	...
	ne.content = function()
		return not state.ontop and icons.ontop_on or icons.ontop_off
	end
	ne.tooltipF = function()
		if user_opts.ontop_in_topbar and window_controls_enabled() and state.ontop then
			return nil
		end
	...
	ne.tooltipF = function()
		return state.file_loop and locale.file_loop_enable or locale.file_loop_disable
	end
	...
	ne.tooltipF = function()
		return state.shuffled and locale.shuffle or locale.unshuffle
	end
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.content = function()
		return string.format(state.speed % 1 == 0 and "%.1f×" or "%g×", state.speed)
	end
	...
	ne.content = function()
		...
		local cache_info = (mp.get_property_bool("paused-for-cache") == true)
				and (locale.buffering .. ": " .. (mp.get_property("cache-buffering-state") or 0) .. "%")
			or cache_time
		if not user_opts.cache_info_speed then
			return cache_info
		end
	...
	ne.tooltipF = function()
		return cache_enabled() and locale.cache or nil
	end
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.slider.tooltipF = function(pos)
		if state.duration ~= nil and pos ~= nil then
			return format_time(state.duration * (pos / 100))
		end
		return ""
	end
	...
	ne.eventresponder["mbtn_left_up"] = function(element)
		...
		if state.playing_and_seeking then
			...
			if
				...
				and not mp.get_property_bool("eof-reached")
...
local function has_click_action(e)
	if not e.eventresponder then
		return false
	end
	for _, k in ipairs(click_keys) do
		if e.eventresponder[k] then
			return true
		end
	end
	return false
...
local function refresh_input_area()
	...
	for n = 1, #elements do
		...
		if e.hitbox and mouse_hit(e) then
			if has_click_action(e) then
				hovered_click = e
			end
			if has_wheel_action(e) then
				hovered_wheel = e
			end
			if has_mid_action(e) then
				hovered_mid = e
			end
		end
	end
	...
	set_virt_mouse_area(
		hovered_click and hovered_click.hitbox.x1 or 0,
		hovered_click and hovered_click.hitbox.y1 or 0,
		hovered_click and hovered_click.hitbox.x2 or 0,
		hovered_click and hovered_click.hitbox.y2 or 0,
		"input"
	)
	set_virt_mouse_area(
		hovered_wheel and hovered_wheel.hitbox.x1 or 0,
		hovered_wheel and hovered_wheel.hitbox.y1 or 0,
		hovered_wheel and hovered_wheel.hitbox.x2 or 0,
		hovered_wheel and hovered_wheel.hitbox.y2 or 0,
		"input_wheel"
	)
	set_virt_mouse_area(
		hovered_mid and hovered_mid.hitbox.x1 or 0,
		hovered_mid and hovered_mid.hitbox.y1 or 0,
		hovered_mid and hovered_mid.hitbox.x2 or 0,
		hovered_mid and hovered_mid.hitbox.y2 or 0,
		"input_mid"
	)
end
		...
		then
			if window_controls_enabled() and user_opts.windowcontrols_independent then
				...
				elseif
					...
					and not state.keeponpause_active
					and user_opts.deadzone_hide ~= "timeout"
				then
					hide_osc()
				end
			else
...
local function render()
	...
	update_input_area("window-controls", state.wc_visible, "windowcontrols_buttons", function()
		mp.enable_key_bindings("window-controls")
	end)
	update_input_area("window-controls-title", state.wc_visible, "windowcontrols_title", function()
		mp.enable_key_bindings("window-controls-title", "allow-vo-dragging")
	end)
	update_input_area("window-controls-ontop", state.wc_visible, "windowcontrols_ontop", function()
		mp.enable_key_bindings("window-controls-ontop")
	end)
...
tick = function()
	...
	local function tick_animation(anitype_key, anistart_key, animation_key, allow_idle)
		if state[anitype_key] ~= nil then
			if
				(allow_idle or not state.idle_active)
...
observe_cached("chapter-list", function()
	...
	table.sort(state.chapter_list, function(a, b)
