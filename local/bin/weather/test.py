...
class WeatherProcessor:
    ...
    def process_daily(
        ...
        df["temperature_2m"] = (
            ...
            + "/"
            + self._fmt_temp(df["temperature_2m_min"])
        )
        df["temperature_2m_max"] = self._fmt_temp(df["temperature_2m_max"])
        df["temperature_2m_min"] = self._fmt_temp(df["temperature_2m_min"])
        df["precipitation_sum"] = self._fmt_size(df["precipitation_sum"])
        df["precipitation_probability"] = self._fmt_probability(
            df["precipitation_probability_max"]
        )
        return df.head(7).reset_index(drop=True)
