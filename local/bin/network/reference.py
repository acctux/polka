...
device = dbus.Interface(
    bus.get_object("net.connman.iwd", sys.argv[1]), "net.connman.iwd.Station"
)
device.Scan()
...
network = dbus.Interface(
    bus.get_object("net.connman.iwd", sys.argv[1]), "net.connman.iwd.Network"
)
network.Connect()
...
for path, interfaces in manager.GetManagedObjects().items():
    ...
    if (network["Name"], network["Type"]) == forget:
        obj = dbus.Interface(
            bus.get_object("net.connman.iwd", path), "net.connman.iwd.KnownNetwork"
        )
        obj.Forget()
        continue
    ...
    for key in network:
        val = network[key]
        if isinstance(val, dbus.Boolean):  # Updated line
